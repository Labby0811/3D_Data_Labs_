...
void BasicSfM::readFromFile(const std::string &filename, bool load_initial_guess, bool load_colors) {
    ...
    std::cout << "Header: " << num_cam_poses_ << " " << num_points_ << " " << num_observations_ << std::endl;
...
void BasicSfM::printPose(int idx) const {
    ...
    std::cout << "camera[" << idx << "]" << std::endl << "{" << std::endl << "\t r_vec : (" << cam[0] << ", " << cam[1]
              << ", " << cam[2] << ")" << std::endl << "\t t_vec : (" << cam[3] << ", " << cam[4] << ", " << cam[5]
              << ")" << std::endl;
...
bool BasicSfM::incrementalReconstruction(int seed_pair_idx0, int seed_pair_idx1) {
    ...
    if (countNonZero(inlier_mask_H) >= countNonZero(inlier_mask_E)) {
        cout << "The number of inliers of H are more than E\n";
        return false;
    }
    ...
    for (auto const &co_iter: cam_observation_[ref_cam_pose_idx]) {
        ...
        if (cam_observation_[new_cam_pose_idx].find(pt_idx) != cam_observation_[new_cam_pose_idx].end()) {
            if (inlier_mask_E.at<unsigned char>(r)) {
                ...
                if (pt[2] > 0.0) {
                    ...
                    cv::Point2d p0(pt[0] / pt[2], pt[1] / pt[2]), p1(pt_3d(0, 0) / pt_3d(2, 0),
                                                                     pt_3d(1, 0) / pt_3d(2, 0));
    ...
    for (int iter = 1; iter < num_cam_poses_ - 1; iter++) {
        ...
        cv::solvePnPRansac(scene_pts, img_pts, intrinsics_matrix, cv::Mat(), init_r_vec, init_t_vec, false, 100,
                           max_reproj_err_);
            ...
            if (cam_pose_optim_iter_[cam_idx] > 0) {
                for (auto const &co_iter: cam_observation_[cam_idx]) {
                    ...
                    if (pts_optim_iter_[pt_idx] == 0 &&
                        ...
                        double *cam0_data = cameraBlockPtr(new_cam_pose_idx), *cam1_data = cameraBlockPtr(cam_idx);
...
*/
        ...
        Eigen::Vector3d vol_min = Eigen::Vector3d::Constant(
                (std::numeric_limits<double>::max())), vol_max = Eigen::Vector3d::Constant(
    ...
    while (keep_optimize) {
        ...
        for (int i_obs = 0; i_obs < num_observations_; i_obs++) {
            ...
            if (cam_pose_optim_iter_[cam_pose_index_[i_obs]] > 0 && pts_optim_iter_[point_index_[i_obs]] > 0) {
                ...
                ceres::CostFunction *cost_function = ReprojectionError::Create(observations_[i_obs * 2],
        ...
        for (int i_obs = 0; i_obs < num_observations_; i_obs++) {
            if (cam_pose_optim_iter_[cam_pose_index_[i_obs]] > 0 && pts_optim_iter_[point_index_[i_obs]] == 1 &&
...
int BasicSfM::rejectOuliers() {
    ...
    for (int i_obs = 0; i_obs < num_observations_; i_obs++) {
        if (cam_pose_optim_iter_[cam_pose_index_[i_obs]] > 0 && pts_optim_iter_[point_index_[i_obs]] > 0) {
            double *camera = cameraBlockPtr(cam_pose_index_[i_obs]), *point = pointBlockPtr(
                    point_index_[i_obs]), *observation = observations_.data() + (i_obs * 2);
